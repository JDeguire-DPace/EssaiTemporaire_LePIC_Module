...
contains
  ...
  subroutine load_particles_legacy(cfg, mpi_rank, mpi_size, n, h, bcnd, kq, ntype_all, ntype_trk, part)
    ...
    integer(int32), intent(in)    :: ntype_all      ! chemistry count (for debug)
    integer(int32), intent(in)    :: ntype_trk      ! charged-only count
    type(ParticleSet), intent(inout) :: part(:,:)   ! <-- FIX: fully assumed-shape
    ...
    ! sanity: caller must have allocated part(ntype_trk, nproc_cfg_or_runtime)
    ...
    flag_inj = int(cfg%flag_inj, int32); if (flag_inj == 0_int32) flag_inj = 1_int32
    opt_inj  = int(cfg%opt_inj,  int32); if (opt_inj  == 0_int32) opt_inj  = 1_int32

    ! ---- VOLUME loading: number of loadable interior cells ----
    ! ---- Legacy-style particle loading region: x <= x_load ----
    ! Legacy prints e.g. "Particle loading region: nx<= 65" which corresponds to ix_load = n(1)+1 for full domain.
    ...
    ! ---- EXACT legacy seeds ----
    do ip = 1, nproc
      iseed(ip) = 123456*ip*(10*mpi_rank + 1)
    end do
    ...
    deallocate(vxp, np_arr, np_tot, Nh, iseed, sum_dEk, ni0_legacy)
