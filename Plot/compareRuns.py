...
def compare_one_dim(
    ...
    im1 = axes[0].imshow(arr1, origin="lower", aspect="auto", extent=extent, vmin=minimum, vmax=maximum)
    plt.colorbar(im1, ax=axes[0])
    axes[0].set_title(f"it{it1}\n{path1.name}")
    axes[0].set_xlabel(xlabel)
    axes[0].set_ylabel(ylabel)

    im2 = axes[1].imshow(arr2_interp, origin="lower", aspect="auto", extent=extent, vmin=minimum, vmax=maximum)
